...
#
out_dir = '/scratch/GW/pool0/gadelrab/scenes'
#
all_methods_results = []
...
kg_triples = kgts.load_from_file('/www/inf-homepage/sreyasi/ExCut/ExCut_triples.txt')
print(kg_triples.as_numpy_array()[:10])

# # Load target entities
target_entities = tes.load_from_file('/www/inf-homepage/sreyasi/ExCut/ExCut_entities.txt')
...
save_model(model, os.path.join(out_dir, 'scenes_transE.pkl'))
################################
...
query_interface = EndPointKGQueryInterfaceExtended(endpoint='http://badr:8890/sparql',
                                          identifiers=['http://scenes.org','http://scenes.org.types'],
                                                       labels_identifier='http://scenes.org.labels.m')
