...
from kg.utils.data_formating import entity_full_url, relation_full_url
from kg.kg_triples_source import TriplesSource, FileTriplesSource
...
class Indexer():
    ...
    def __init__(self, store='remote', endpoint=None, identifier=None, graph=None, batch_size=100, remove_invalid_ids=True):
        self.remove_invalid_ids = remove_invalid_ids
        self.batch_size = batch_size
        self.store = 'SPARQLUpdateStore' if store=='remote' else 'default'
    ...
    def insert_sparql(self, triples):
        triples_filtered= filter(lambda a: data_formating.valid_id_triple(a),triples) if self.remove_invalid_ids else triples
...
if __name__ == '__main__':
    ...
    indexer.index_triples(FileTriplesSource('/home/gadelrab/ExDEC/data/20k_kemans_it1.nt', prefix='http://test.org/', safe_urls=True),
                          drop_old=True)
    c=0
