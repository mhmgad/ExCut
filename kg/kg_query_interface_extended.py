...
from explanations_mining.descriptions_new import Atom, Description2
from kg.utils.data_formating import _sparql_repr
        ...
        for j in range(0, description.size() - 1):
            ...
            if atom.predicate == other_atom.predicate:
                if atom.subject == other_atom.subject:
                    uniq_atoms_filter.append('FILTER(' + _sparql_repr(atom.object) + '!=' + _sparql_repr(other_atom.object) + ').')
                elif atom.object == other_atom.object:
                    uniq_atoms_filter.append('FILTER(' + _sparql_repr(atom.subject) + '!=' + _sparql_repr(other_atom.subject) + ').')
            if atom.subject == other_atom.subject and atom.object == other_atom.object:
                uniq_atoms_filter.append('FILTER(' + _sparql_repr(atom.predicate) + '!=' + _sparql_repr(other_atom.predicate) + ').')
        return uniq_atoms_filter
