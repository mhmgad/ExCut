...
from explanations_mining.simple_miner.description_miner import DescriptionMiner
from explanations_mining.descriptions import top
from kg.kg_query_interface import EndPointKGQueryInterface, KGQueryInterface
from kg.utils.Constants import DEFUALT_AUX_RELATION
from clustering.target_entities import EntitiesLabelsFile


class ClustersExplainer:
    """
    Class that takes a set of clusters and generate explanations for them
    """

    def __init__(self):
        pass

    # def _explain(self, clusters, clusters_relation,output_file=None):
    #     """
    #     Learn explanations for list of clusters and rank them.
    #
    #     :param clusters:
    #     :param output_file:
    #     :return:
    ...
    def explain(self, entity_2_clusters_triples, output_file=None):
        """
        Learn explanations for entities and their clusters represented as triples  and rank them.
    ...
    def _prepare_data(self, clusters_as_triples):
        """
        Prepare data for mining explanations such as indexing

        :param clusters_as_triples: entity-clusters labels
        :return: None
        """
        pass
    ...
    def clear_data(self):
        """
        Clear any indexed data.
        :return:
        """
        pass


class PathBasedClustersExplainer(ClustersExplainer):

    def __init__(self, kg_query_interface: KGQueryInterface, labels_indexer=None, relation=DEFUALT_AUX_RELATION,
                 min_coverage=0.4,
        ...
        self.description_miner = DescriptionMiner(kg_query_interface, per_pattern_binding_limit,
    ...
    def _explain(self, clusters, clusters_relation,output_file=None):
        ...
        heads = [('?x', clusters_relation, cl) for cl in clusters]
        ...
        top_cls_descriptions = {h[2]: top(des, self.top, method=self.quality_method) for h, des
                                in
                                cls_descriptions.items()}
    ...
    def explain(self, entity_2_clusters_triples, output_file=None, clear=True):
        ...
        top_cls_descriptions= self._explain(entity_2_clusters_triples.get_uniq_labels(), relation,output_file=output_file)
        if clear:
            self.clear_data()
        return top_cls_descriptions
