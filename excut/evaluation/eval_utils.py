...
from excut.evaluation.embedding_metrics import compute_avg_sim

mpl.use('Agg')
import matplotlib.pyplot as plt
import pandas as pd

import excut.evaluation.clustering_metrics as clsm
import excut.evaluation.explanations_metrics as explm
from excut.explanations_mining.descriptions import load_from_file_dict
...
default_results_headers = ['expr_name', 'itr_num'] + sorted(list(clsm.evals_methods.keys())) + \
...
def export_evaluation_results(results_list, out_filepath, headers=None, append=False):
    """
    ...
    with open(out_filepath, 'a' if append else 'w') as iter_stats_file:
...
def evaluate_iterations_from_files(gt_file, input_folder, objective_measure=None, save=False, dataset_name=''):
    ...
    iter_stats_plot_file = os.path.join(input_folder,
                                        'iters_stats_plot_%s_from_files.pdf' % dataset_name) if save else None
