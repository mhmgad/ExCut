...
from excut.clustering.target_entities import EntityLabelsInterface, EntitiesLabelsFile, align_entity_labels_triples
...
evals_methods = {'ACC': acc_micro, 'NMI': nmi, 'ARI': ari, 'HMS': hms, 'PUR': purity_score,
                 'SS': silhouette_width_score}


def evalaute_triples(ground_truth_triples: EntityLabelsInterface, predicted_labels_triples: EntityLabelsInterface,
                     fill_missing=True):
    """
...
def evaluate(y_true, y_pred, input_vectors=[], metric=None, verbose=False):
    """
    ...
    if len(y_true) != 0:
        ...
        res.update({'ACC': acc_micro(y_true, y_pred, verbose),
    ...
    if len(input_vectors) > 0:
        res.update({
            'SS': silhouette_width_score(input_vectors, y_pred, metric=metric)
        })
